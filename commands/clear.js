...
module.exports = {
	...
	execute(message, args) {
        if (!message.member.roles.cache.some(role => role.id == moderatorRoleID)) message.delete().then(message.channel.send("This command is only available to moderators.")).catch((error)=>console.error(error))
