	...
	execute(message, args) {
		if (!message.member.roles.cache.some((role) => role.id == moderatorRoleID))
			message
				...
				.then(
					message.channel
						.send('This command is only available to moderators.')
						.then((message) => message.delete({ timeout: messageTimeout }))
				)
			...
			message
				...
				.then(
					message.channel
						.send('You need to specify how many messages you want to delete.')
						.then((message) => message.delete({ timeout: messageTimeout }))
				)
				.catch((error) => console.error(error));
		else
