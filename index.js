...
client.on("guildMemberAdd", (member) => {
  client.channels.cache
    ...
    .send(
      `**${member.user.username}** joined the server. \`${member.guild.memberCount}\``
    );
  if (member.user.bot)
    member.roles.add(
      member.guild.roles.cache.find((role) => role.id == "731568194552070184")
    );
  // add bot role
  else
    member.roles.add(
      member.guild.roles.cache.find((role) => role.id == "731570521690472508")
    ); // add member role
});
client.on("guildMemberRemove", (member) =>
  client.channels.cache
    .get("751093731171762188")
    .send(
      `**${member.user.username}** left the server. \`${member.guild.memberCount}\``
    )
);
client.on("messageReactionAdd", async (reaction, user) => {
  ...
  if (reaction.message.channel.id == verifyChannelID)
    await reaction.message.guild.members.cache
      .get(user.id)
      .roles.add(verifiedRoleID);
  client.channels.cache
    .get(roleDataChannelID)
    ...
    .then(async (messages) => {
      ...
      if (
        reaction.message.channel.id == roleChannelID &&
        availableReactions.includes(reaction.emoji.name)
      ) {
        await reaction.message.guild.members.cache
          .get(user.id)
          .roles.add(
            reactionsConfig[availableReactions.indexOf(reaction.emoji.name)]
              .role
          );
      }
    });
  ...
  const starboardChannel = client.channels.cache.find(
    (c) => c.id == starboardChannelID
  );
  ...
  if (reaction.emoji.name == "⭐" && reaction.count >= 3) {
    ...
    if (sentMessage == null) {
      ...
      await starboardChannel
        ...
        .then(
          async (message) =>
            (msg = await starboardChannel.messages.fetch(message.id))
        );
      // console.log(msg);
      starboard.set(message.id, {
        originalMessage: message,
        starboardMessage: msg,
      });
    } else {
      const oldMessage = await starboardChannel.messages.fetch(
        starboard.get(message.id).starboardMessage[0].id
      );
